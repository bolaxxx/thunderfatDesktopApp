 ...
 * @author sergio
 */
public class ListCitaWindow extends JInternalFrame {
    private static ListCitaWindow listCitaWindow;
	private ControlListCitaWindow control;
...
//	public JComboBox<String> userTypeComboBox;
	...
	public JButton addUser;
        ...
        private ListCitaWindow(ControlListCitaWindow c, MyTableModel m){
...
//		this.userPanel.add(userTypeComboBox,BorderLayout.EAST);
	...
	public static ListCitaWindow getInstance(ControlListCitaWindow c, MyTableModel m){
		if (listCitaWindow == null){
			listCitaWindow = new ListCitaWindow(c,m);
		}
		
		if (listCitaWindow.isVisible()) {
			try {
				listCitaWindow.setSelected(true);
			} catch (PropertyVetoException e) {
				e.printStackTrace();
			}
		} else {
			listCitaWindow.setVisible(true);
		}	

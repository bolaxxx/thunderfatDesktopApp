...
import control.ControlListRecetasWindow;
 ...
 * @author sergio
 */
public class ListRecetasWindow extends JInternalFrame {
      private static ListRecetasWindow listRecetaWindow;
	private ControlListRecetasWindow control;
...
//	public JComboBox<String> userTypeComboBox;
	...
	public JButton addUser;
         ...
         private ListRecetasWindow(ControlListRecetasWindow c, MyTableModel m){
        	super("Lista Cita",true,true,true,true);
		this.control = c;				
...
//		this.userPanel.add(userTypeComboBox,BorderLayout.EAST);
	...
	public static ListRecetasWindow getInstance(ControlListRecetasWindow c, MyTableModel m){
		if (listRecetaWindow == null){
			listRecetaWindow = new ListRecetasWindow(c,m);
		}
		
		if (listRecetaWindow.isVisible()) {
			try {
				listRecetaWindow.setSelected(true);
			} catch (PropertyVetoException e) {
				e.printStackTrace();
			}
		} else {
			listRecetaWindow.setVisible(true);
		}	
